...
* Abstract
  ...
  appreciated as it would cause online banking to be easily hacked. If a
  polynomial time algorithm would be found to the 3-SAT problem it would also
  apply to all problems in the complexity class NP. A polynomial algorithm
  to 3-SAT, or simply a proof of the existence or non-existence of one is yet
  to be found. Although advances in machine self-learning with the use of neural
  networks has lately shown much succes. Many advanced games such as Go and DOTA
  can self taught AIs outperform. In this paper, neural networks are trained
  to solve fixed variable lenght 3-SAT problems in order to see what the results
  sugest regarding the finding of a polynomial time algorithm.
* Inledning
** Bakgrund
...
*** Python
    ...
    och viktigaste funktionen vid lärning av python är funktionen "print". 
